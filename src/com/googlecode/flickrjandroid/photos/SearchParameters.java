...
public class SearchParameters {
	 ...
	 * factor is passed flickr will return only photos added in the last 12
	 * hours (though flickr may extend the limit in the future).
	 * 
	 * @param hasGeo
	 */
	...
	/**
	 * List of extra information to fetch for each returned record. Currently
	 ...
	 * o_dims, views, media, path_alias, url_sq, url_t, url_s, url_m, url_l,
	 * url_o
	 * 
	 * @param extras
	 ...
	 * <p>
	 * 
	 * Longitude has a range of -180 to 180, latitude of -90 to 90. Defaults to
	 * -180, -90, 180, 90 if not specified.
	 * <p>
	 * 
	 ...
	 * passed flickr returns only photos added in the last 12 hours (though
	 * flickr may extend the limit in the future).
	 * 
	 ...
	 */
	public void setBBox(String minimum_longitude, String minimum_latitude,
			String maximum_longitude, String maximum_latitude) {
		this.bbox = new String[] { minimum_longitude, minimum_latitude,
	 ...
	 * A tag, for instance, is considered a limiting agent as are user defined
	 * min_date_taken and min_date_upload parameters &emdash; If no limiting
	 * factor is passed we return only photos added in the last 12 hours (though
	 * we may extend the limit in the future).
	 * 
	 * @param placeId
	 * @see com.googlecode.flickrjandroid.places.PlacesInterface#resolvePlaceId(String)
	 ...
	 * For example all the photos clustered by locality in the United States
	 * (WOE ID 23424977).<br>
	 ...
	 * A tag, for instance, is considered a limiting agent as are user defined
	 * min_date_taken and min_date_upload parameters. If no limiting factor is
	 * passed we return only photos added in the last 12 hours (though flickr
	...
	public void setMedia(String media) throws FlickrException {
		if (media.equals("all") || media.equals("photos")
				|| media.equals("videos")) {
			this.media = media;
		} else {
	...
	/**
	 * Search your contacts. Valid arguments are either 'all' or 'ff' for just
	 * friends and family.
	 * <p/>
	 * 
	...
	public Collection<Parameter> getAsParameters() {
		...
		if (mtags != null) {
			parameters.add(new Parameter("machine_tags", StringUtilities.join(
					mtags, ",")));
		}
		...
		if (minUploadDate != null) {
			parameters.add(new Parameter("min_upload_date", new Long(
					minUploadDate.getTime() / 1000L)));
		}

		Date maxUploadDate = getMaxUploadDate();
		if (maxUploadDate != null) {
			parameters.add(new Parameter("max_upload_date", new Long(
					maxUploadDate.getTime() / 1000L)));
		}

		Date minTakenDate = getMinTakenDate();
		if (minTakenDate != null) {
			parameters.add(new Parameter("min_taken_date",
					((DateFormat) MYSQL_DATE_FORMATS.get())
							.format(minTakenDate)));
		}

		Date maxTakenDate = getMaxTakenDate();
		if (maxTakenDate != null) {
			parameters.add(new Parameter("max_taken_date",
					((DateFormat) MYSQL_DATE_FORMATS.get())
							.format(maxTakenDate)));
		}
		...
		if (intrestingnessDate != null) {
			parameters.add(new Parameter("date", ((DateFormat) DATE_FORMATS
					.get()).format(intrestingnessDate)));
		}

		String[] bbox = getBBox();
		if (bbox != null) {
			parameters.add(new Parameter("bbox", StringUtilities
					.join(bbox, ",")));
			if (accuracy > 0) {
